 ...
 */
function executeUnifiedSearch(content, pattern, useRegex = true, flags = 'g') {
    try {
 ...
 */
function filterFlagsForMode(flags, useRegex) {
 ...
 */
function executeSearchReplace(content, searchPattern, replacePattern, useRegex = true, flags = 'g') {
    try {
 ...
 */
function escapeSearchQueryLiteral(value) {
    return value.replace(/'/g, "''");
}
/**
 * Check if a note with the same title already exists in the same directory
 */
async function checkDuplicateTitleInDirectory(parentNoteId, title, axiosInstance) {
 ...
 */
export async function handleUpdateNote(args, axiosInstance) {
    const { noteId, title, type, content: rawContent, mime, fileUri, revision = true, expectedHash, mode } = args;
    ...
    try {
        ...
        let existingTemplateRelation;
        try {
            // Check if the note has existing template relations
            const existingAttributes = currentNote.data.attributes || [];
            existingTemplateRelation = existingAttributes.find((attr) => attr.type === 'relation' && attr.name === 'template')?.value;
        }
        catch (error) {
        ...
        const validationResult = await validateContentForNoteType(rawContent, type, currentContent.data, existingTemplateRelation);
 ...
 */
export async function handleSearchReplaceNote(args, axiosInstance) {
    const { noteId, searchPattern, replacePattern, useRegex = true, searchFlags = 'g', revision = true, expectedHash } = args;
    if (!noteId) {
    ...
    try {
        ...
        const { newContent, replacements } = executeSearchReplace(originalContent, searchPattern, replacePattern, useRegex, searchFlags);
        ...
        const validationResult = await validateContentForNoteType(newContent, noteType, originalContent);
        if (!validationResult.valid) {
            return {
                noteId,
 ...
 */
export async function handleDeleteNote(args, axiosInstance) {
    const { noteId } = args;
    if (!noteId) {
 ...
 */
export async function handleListChildrenNotes(args, axiosInstance) {
    const { noteId } = args;
    if (!noteId) {
    ...
    const children = (response.data.results || []).map((note) => ({
        noteId: note.noteId,
        title: note.title,
        type: note.type,
        mime: note.mime || '',
...
export async function handleSyncSubtreeContent(args, axiosInstance) {
    const { sourceRootNoteId, targetRootNoteId, dryRun = false, matchBy = 'title', recursive = true, overwriteTitles = false, createRevision = true, skipMissingTargets = true } = args;
    if (!sourceRootNoteId || !targetRootNoteId) {
        throw new Error("sourceRootNoteId and targetRootNoteId are required for sync_subtree_content operation.");
    }
    if (matchBy !== 'title') {
        throw new Error("Only matchBy='title' is currently supported.");
    }
    const fetchNoteAndContent = async (noteId) => {
        const [noteResp, contentResp] = await Promise.all([
            axiosInstance.get(`/notes/${noteId}`),
            axiosInstance.get(`/notes/${noteId}/content`, { responseType: 'text' }).catch(() => ({ data: '' }))
        ]);
        return { note: noteResp.data, content: contentResp.data || '' };
    };
    const listChildrenDetailed = async (noteId) => {
        const listed = await handleListChildrenNotes({ noteId }, axiosInstance);
        return listed.children || [];
    };
    const shouldSyncContentType = (type) => ['text', 'code', 'mermaid'].includes(type);
    const summary = {
        sourceRootNoteId,
        targetRootNoteId,
        dryRun,
    ...
    const walk = async (sourceId, targetId, pathTitles = []) => {
        const [{ note: sourceNote, content: sourceContent }, { note: targetNote, content: targetContent }] = await Promise.all([
            fetchNoteAndContent(sourceId),
            fetchNoteAndContent(targetId)
        ]);
        const currentPath = [...pathTitles, sourceNote.title];
        if (sourceNote.type !== targetNote.type) {
            ...
            summary.actions.push({
                ...
                sourceType: sourceNote.type,
                targetType: targetNote.type
            });
        } else if (shouldSyncContentType(sourceNote.type)) {
            summary.matched += 1;
            const titleChanged = overwriteTitles && sourceNote.title !== targetNote.title;
            const contentChanged = sourceContent !== targetContent;
            if (!titleChanged && !contentChanged) {
                ...
                try {
                    await handleUpdateNote({
                        noteId: targetId,
                        title: overwriteTitles ? sourceNote.title : undefined,
                        type: targetNote.type,
                        mime: targetNote.mime,
                        content: sourceContent,
                        expectedHash: targetNote.blobId,
        ...
        const [sourceChildren, targetChildren] = await Promise.all([
            listChildrenDetailed(sourceId),
            listChildrenDetailed(targetId)
        ]);
        const targetByTitle = new Map();
        for (const child of targetChildren) {
            if (!targetByTitle.has(child.title)) targetByTitle.set(child.title, []);
            targetByTitle.get(child.title).push(child);
        }
        for (const sChild of sourceChildren) {
            const candidates = targetByTitle.get(sChild.title) || [];
            if (candidates.length === 0) {
                summary.missingTargets += 1;
                summary.actions.push({
                    path: [...currentPath, sChild.title].join(' / '),
                    sourceNoteId: sChild.noteId,
                    targetNoteId: null,
                    status: skipMissingTargets ? 'missing_target_skipped' : 'missing_target'
                });
                continue;
            }
            await walk(sChild.noteId, candidates[0].noteId, currentPath);
        }
    };
    await walk(sourceRootNoteId, targetRootNoteId, []);
    return summary;
}
 ...
 */
export async function handleMoveNote(args, axiosInstance) {
    ...
    const parentBranchIds = noteResponse.data.parentBranchIds || [];
    ...
    if (parentBranchIds.length === 1) {
        ...
        if (!parentBranchIds.includes(providedBranchId)) {
            throw new Error(`Branch ${providedBranchId} does not belong to note ${noteId}. ` +
        ...
        const branchDetails = await Promise.allSettled(parentBranchIds.map((bid) => {
            logVerboseApi("GET", `/branches/${bid}`);
            return axiosInstance.get(`/branches/${bid}`);
        }));
        const branches = branchDetails.map((r, idx) => {
        ...
        return {
            noteId,
            message: `Note ${noteId} exists in ${parentBranchIds.length} locations. ` +
    ...
    const oldParentNoteId = branchResponse.data.parentNoteId;
    // No-op check
    ...
    let existingDestBranchId = null;
    if (otherBranchIds.length > 0) {
        const otherBranchDetails = await Promise.allSettled(otherBranchIds.map(bid => {
            logVerboseApi("GET", `/branches/${bid}`);
            return axiosInstance.get(`/branches/${bid}`);
        }));
    ...
    if (existingDestBranchId) {
        ...
        try {
            ...
            if (deleteError?.response?.status === 404) {
                ...
                throw new Error(`Move failed: note ${noteId} is already a child of ${newParentNoteId} (via ${existingDestBranchId}) ` +
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Move failed and rollback failed: note ${noteId} now exists in both ${oldParentNoteId} and ${newParentNoteId}. ` +
                `Manually delete one of the branches (old: ${targetBranchId}, new: ${newBranchId}).`);
        }
        throw new Error(`Move failed: could not delete old branch ${targetBranchId} from ${oldParentNoteId}. ` +
 ...
 */
function normalizeFlags(flags, scope) {
 ...
 */
function isLiteralContext(value) {
 ...
 */
function literalMatchHasContext(content, candidate, context) {
 ...
 */
function validatePatchShape(patch, patchIndex) {
 ...
 */
function applyHtmlPatch(content, patch, patchIndex) {
 ...
 */
function applyLinePatch(content, patch, patchIndex) {
    ...
    if (/^\d+$/.test(selector)) {
        ...
        const matches = lines.reduce((acc, line, idx) => (line.includes(selector) ? [...acc, idx] : acc), []);
        if (matches.length === 0) {
 ...
 */
function applyLiteralPatch(content, patch, patchIndex) {
    const scope = patch.scope ?? 'one';
 ...
 */
function applySearchPatch(content, patch, patchIndex) {
    const scope = patch.scope ?? 'one';
    ...
    try {
        ...
        throw new Error(`patches[${patchIndex}].selector '${patch.selector}' failed during regex search/replace: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (matchCount === 0) {
        throw new Error(`patches[${patchIndex}].selector '${patch.selector}' matched no text.`);
    }
 ...
 */
export async function handlePatchNote(args, axiosInstance) {
    ...
    const currentBlobId = noteData.blobId;
    if (currentBlobId !== expectedHash) {
        return {
            noteId,
            revisionCreated: false,
    ...
    if (!PATCHABLE_NOTE_TYPES.has(noteData.type)) {
        throw new Error(`patch_note only supports text, code, and mermaid note types. Note ${noteId} is type '${noteData.type}'.`);
    }
    ...
    const validationResult = await validateContentForNoteType(workingContent, noteData.type, originalContent);
    if (!validationResult.valid) {
 ...
 */
export async function handleGetNote(args, axiosInstance) {
    const { noteId, includeContent = true, includeBinaryContent = false, searchPattern, useRegex = true, searchFlags = 'g' } = args;
    if (!noteId) {
    ...
    if (searchPattern) {
        ...
        const enhancedMatches = matches.map(match => ({
            ...match,
            htmlContext: {
                contentType: (contentRequirements.requiresHtml ? 'html' : 'plain'),
