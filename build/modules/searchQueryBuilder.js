 ...
 * Handles all search criteria types: labels, relations, and note properties
 * Enables previously impossible cross-type OR operations
 */
function buildUnifiedSearchExpressions(searchCriteria) {
    const expressions = [];
    let currentGroup = [];
    let groupLogic = 'AND'; // Default to AND as per TriliumNext behavior
 ...
 */
function buildSearchCriteriaQuery(criteria) {
    const { type } = criteria;
    switch (type) {
        case 'label':
        case 'relation':
 ...
 */
function finalizeGroup(queries, logic) {
    if (queries.length === 1) {
        return queries[0];
    }
    if (logic === 'OR') {
 ...
 */
function buildAttributeQuery(criteria) {
 ...
 */
function enhanceRelationProperty(name, op, value) {
 ...
 */
function isTemplateOrSystemRelation(property) {
 ...
 */
function validateNoteType(value) {
 ...
 */
function validateISODate(value, property) {
 ...
 */
function buildNotePropertyQuery(criteria) {
